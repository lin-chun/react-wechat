    ...
    for (let i = 0; i < detail.annexationList.length; i++) {
      if (detail.annexationList[i].annexationInfoName == '产品相关协议') {
        for (let j = 0; j < detail.annexationList[i].list.length; j++) {
          if (detail.annexationList[i].list[j].annexationInfoName == '产品合同') {
            this.setState({
          ...
          if (detail.annexationList[i].list[j].annexationInfoName == '风险揭示') {
            this.setState({
              risk: detail.annexationList[i].list[j].annexationInfo
            });
          };
          if (detail.annexationList[i].list[j].annexationInfoName == '委托人申明') {
            this.setState({
              declar: detail.annexationList[i].list[j].annexationInfo
            });
          };
          if (detail.annexationList[i].list[j].annexationInfoName == '产品说明') {
            this.setState({
              instructions: detail.annexationList[i].list[j].annexationInfo
            });
          };
        }
      }
    ...
    return (
      <div className="v-agreement">
        <Cells className="notice-list">
          ...
          <Cell href={ this.state.instructions } access>
            <CellBody>产品说明</CellBody>
            <CellFooter />
          </Cell>
